        ...
        if($result2->num_rows > 0){
            ...
            if ($result3->num_rows > 0) {
                ...
                if (!preg_match('/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&£#])[A-Za-z\d@$!%*?&£#]{8,}$/', $password)){
                    ...
                    $sql = "INSERT INTO users (idno, lastname, firstname, middlename, course, level, email, username, password, role) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    $stmt = $conn->prepare($sql);
                    $stmt->bind_param("issssissss", $idno, $lastname, $firstname, $middlename, $course, $level, $email, $username, $password, $role);
